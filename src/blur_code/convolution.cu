...
//not need to padding
__global__ void conv_x(GPU::PtrStepSz<uchar3> src,/*const double* __restrict__ guass_kernel,*/GPU::PtrStepSz<uchar3> dst,int kernel_size,int kernel_radius,int orign_width,int orign_height){
...
__global__ void conv_y(GPU::PtrStepSz<uchar3> src,/*const double* __restrict__ guass_kernel,*/GPU::PtrStepSz<uchar3> dst,int kernel_size,int kernel_radius,int orign_width,int orign_height){
	__shared__ float  share_mem[100][100];
	/*int pixel_i=blockDim.x*blockIdx.x+threadIdx.x;
	int pixel_j=blockDim.y*blockIdx.y+threadIdx.y;
